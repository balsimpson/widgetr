...
| 4 Sep 2026 | Homepage readiness-gated entry | Gated the new-visitor Studio CTA on observable page-action readiness: unavailable and registration-error states now offer concrete setup guidance, checking and registering states wait without a misleading CTA, ready and working states open `/studio?new=1`, and saved projects retain `Continue` with review-only copy when an assistant is unavailable. Updated the shared status descriptions and product, homepage, and phase documentation. `npm run repo:check`, `git diff --check`, local Nuxt SSR/HMR HTTP 200, and the Impeccable detector passed; the changed route was queued in the Codex in-app browser for handoff. Commit `127c731` is pushed to `origin/main`. | Rendered 1280 x 900 client-state verification, tests, typecheck, production build/deployment, external WebMCP invocation, export, and Scriptable/iPhone execution remain open |
