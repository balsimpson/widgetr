...
| Local branch | `main` |
| Local and remote commit | `HEAD` (no commit created; working tree adds the homepage and `/studio` split, safe onboarding catalogs, shared readiness UI, the official Scriptable asset, and the in-context Studio starter modal while preserving the existing starter changes) |
