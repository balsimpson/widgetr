...
| Local branch | `main` |
| Local and remote commit | `main` and `origin/main` are synchronized at `536ae48`; the dark-mode foreground contrast fix is committed and pushed |
| Latest verified UI commit | `536ae48` (theme-aware dark-mode foreground contrast for the Studio Export and active size controls); the production rollout is verified |
...
| Homepage follow-up | The homepage now passes through WebMCP status, registered tool names, and retry state; shows a compact `Page actions` disclosure; removes the provider chooser; routes new visitors to `/studio?new=1`; and uses a short assistant prompt and onboarding result. `npm test` passes 8 files and 74 tests, and `npm run typecheck` passes. The changed local page, deployed version, and one real external invocation still need verification. |
