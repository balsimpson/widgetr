...
| Local branch | `main` |
| Local and remote commit | Local `main` is at `e5a984b` and is one commit ahead of `origin/main`; the readiness-gating change is committed locally and pending push |
| Latest verified UI commit | `e5a984b` records the homepage readiness-gating change; the deployed version and external WebMCP path still need verification |
...
| Homepage follow-up | The homepage now passes through WebMCP status, registered tool names, and retry state; shows a compact `Page actions` disclosure; removes the provider chooser; gates new-visitor `Start in Studio` on registered or working page actions; shows setup guidance instead when page actions are unavailable or fail; keeps saved-project `Continue` available for review; and uses a short assistant prompt and onboarding result. `npm run repo:check`, `git diff --check`, the local Nuxt SSR/HMR smoke check, and the Impeccable detector pass. Implementation commit `e5a984b` is committed locally and pending push; tests, typecheck, production build, deployment, and one real external invocation remain separate checkpoints. |
...
- [x] Vercel deployment at `https://widgetr.vercel.app/`
...
- [x] Refresh the README so it describes the verified product rather than an earlier phase snapshot.
...
State: Partially implemented locally (reference-image intake slice); broader Phase 6 work and verification remain open
...
- Unavailable guidance names a useful next action such as using a WebMCP-capable environment or copying the assistant message; it does not identify or rank an assistant provider.
...
| 2 Sep 2026 | Studio UX revision | Converted the route-level new-widget chooser into an in-context modal. The Studio now keeps its shell and a neutral empty-canvas state visible, derives the empty state from hydrated project data, opens the starter modal after the last saved project is removed or when `new=1` is requested, and lets Cancel/Escape dismiss it safely. The visible local in-app browser at `1280 x 900` confirmed the dimmed Studio shell behind the modal, compact starter choices, accessible dialog labeling, and no duplicated homepage copy; `git diff --check`, `npm run repo:check`, and the Impeccable detector passed. Targeted ESLint could not run because npm could not reach the registry; tests, typecheck, production build, deployment, real external WebMCP tool invocation, and Scriptable/iPhone execution remain unverified; this follow-up is currently uncommitted |
...
| 3 Sep 2026 | Site favicon and metadata | Added global SVG and PNG favicon links plus route-aware title, description, canonical, Open Graph, and Twitter metadata for `/` and `/studio`, including Widgetr's 1200x630 social preview and large-card type. `npm run repo:check` and `git diff --check` passed; local SSR head checks confirmed the expected tags and image URL on both routes | Tests, typecheck, production build, deployment, and crawler/social-card validation remain deferred |
| 3 Sep 2026 | Shared locale and text fitting | Removed UTC and fixed-English forcing from the shared browser value formatter, aligned date-only weekday parsing and Scriptable date formatting with device locale behavior, and added a shared responsive single-line text fit for constrained values without changing Weather or Bitcoin fixtures. Browser checks at `1280 x 900` confirmed local-time output and complete primary values across both starters and all available sizes; `npm run repo:check` and `git diff --check` passed | Tests, typecheck, production build, deployment, and real Scriptable/iPhone execution remain deferred |
| 4 Sep 2026 | Reference-image assistant intake | Selecting an image in the starter now immediately creates and persists a reference-led project, opens the existing assistant handoff, and exposes `widgetr_get_reference_intake` with a first question, suggested replies, and type-specific follow-up guidance. The assistant prompt and context payload now carry the same handoff contract; `npm run repo:check`, `git diff --check`, and the local Studio route check passed. Feature commit `f5ac9d8` is pushed to `origin/main`. | Starter file-selection interaction, external WebMCP invocation, tests, typecheck, production build, deployment, and Scriptable/iPhone execution remain unverified |
| 4 Sep 2026 | Canvas collaboration attribution | Tagged mutations explicitly at the shared Studio operation boundary so direct edits remain `You` and WebMCP edits remain `Assistant`, even when both can act concurrently. Added transient solid violet user and dashed teal assistant traces to affected previews, actor-aware receipts, and actor labels in History without changing `WidgetProject` or operation schemas. Local Brave checks at `1280 x 900` covered a manual inspector edit and a real Widgetr page action invoked through a temporary page-local WebMCP mock; both paths rendered the correct attribution with zero horizontal overflow and no Vite error overlay. `npm run repo:check`, `git diff --check`, and the Impeccable detector passed. | The real external assistant path, tests, typecheck, production build, deployment, and Scriptable/iPhone execution remain unverified; the change is uncommitted |
| 4 Sep 2026 | Dark-mode foreground contrast | Replaced the Studio's hard-coded white foreground on lavender `Export` and active size controls with a theme-aware accent foreground token: white in light mode and the graphite stage color in dark mode. Local and production browser checks at `1280 x 900` measured `6.87:1` contrast, confirmed zero horizontal overflow, and found no production console diagnostics. Commit `536ae48` is pushed to `origin/main`, and the automatic Vercel rollout is verified at `widgetr.vercel.app`. | Focused tests, typecheck, deployed WebMCP, export, and Scriptable/iPhone execution remain separate open gates |
| 4 Sep 2026 | Homepage WebMCP handoff | Added the compact homepage readiness row, registered page-action disclosure, retry path, direct `Start in Studio`/`Continue` routing, short assistant message, and trimmed onboarding result; removed the provider chooser. `npm run repo:check`, `git diff --check`, `npm test` (8 files, 74 tests), and `npm run typecheck` passed. A local Nuxt SSR/HMR smoke check at `http://127.0.0.1:3100/` confirmed the changed output; implementation commit `9b37b26` is pushed to `origin/main`. | Rendered viewport/console/overflow verification, deployed rollout, one real deployed WebMCP invocation, export, and Scriptable/iPhone checks remain open |
| 4 Sep 2026 | WebMCP metadata injection cleanup | Rewrote the `widgetr_get_started` and `widgetr_create_widget` descriptions to state their returned data and behavior without instructing the agent how to act. `npm run repo:check` and `git diff --check` passed; the local homepage loaded without browser errors, but this browser/model could not list WebMCP tools and `document.modelContext` was unavailable. | Commit, deployment, and the Ora registry audit remain pending |
| 4 Sep 2026 | Homepage readiness-gated entry | Gated the new-visitor Studio CTA on observable page-action readiness: unavailable and registration-error states now offer concrete setup guidance, checking and registering states wait without a misleading CTA, ready and working states open `/studio?new=1`, and saved projects retain `Continue` with review-only copy when an assistant is unavailable. Updated the shared status descriptions and product, homepage, and phase documentation. `npm run repo:check`, `git diff --check`, local Nuxt SSR/HMR HTTP 200, and the Impeccable detector passed; the changed route was queued in the Codex in-app browser for handoff. Commit `e5a984b` is local and pending push. | Push, rendered 1280 x 900 client-state verification, tests, typecheck, production build/deployment, external WebMCP invocation, export, and Scriptable/iPhone execution remain open |
