...
| Local branch | `main` |
| Local and remote commit | `c0bcc77 = origin/main` (the live Bitcoin starter and its related UI, export, tests, and documentation are committed and pushed; the current working tree adds the site favicon, route metadata and 1200x630 social preview pass, shared viewer-locale formatting, generic single-line text fitting, and the user-prioritized reference-image assistant-intake slice) |
| Latest verified UI commit | `c0bcc77` (the pushed live Bitcoin starter); the current working-tree metadata, display, social-card, and reference-intake revision is browser-verified on `/` and `/studio` for the earlier metadata/display behavior and the new local social-card tags, while the reference-intake work remains uncommitted and needs its own interaction verification |
...
| Latest local browser evidence | At `1280 x 900`, the Bitcoin starter created from the modal loaded a live CoinGecko BTC / USD price and seven-point seven-day sparkline; Small, Medium, and Large all rendered the live values, the Tools refresh action completed successfully, and the generated Export modal contained the CoinGecko endpoints with no blocking export state. The current `/` and `/studio` head checks expose the SVG favicon, PNG fallback, language, canonical URL, route-specific description, Open Graph, and Twitter metadata, including the absolute 1200x630 `widgetr-social-preview.png` image and `summary_large_image` card type. The shared display check ran in the browser's `Asia/Kolkata` timezone: Bitcoin `17:07Z` rendered as `10:37 PM`, Weather `04:45Z` rendered as `10:15 AM`, and all Small, Medium, and Large primary prices/temperatures had content widths at or below their element widths after the generic fit pass. The earlier Open-Meteo five-day Ladakh document still rendered without overflow with source-local `Asia/Kolkata` time in Small and Large, and an agent-style Large-element selection preserved the person's `All` preview view. The current local `/studio?new=1` route returned HTTP 200; the new reference-selection interaction and external WebMCP invocation remain unverified. Typecheck, tests, production, and iPhone execution remain unverified. |
...
| 4 Sep 2026 | Reference-image assistant intake | Selecting an image in the starter now immediately creates and persists a reference-led project, opens the existing assistant handoff, and exposes `widgetr_get_reference_intake` with a first question, suggested replies, and type-specific follow-up guidance. The assistant prompt and context payload now carry the same handoff contract; `npm run repo:check`, `git diff --check`, and the local Studio route check passed | Starter file-selection interaction, external WebMCP invocation, tests, typecheck, production build, deployment, and Scriptable/iPhone execution remain unverified |
