...
| 3 Sep 2026 | Site favicon and metadata | Added global SVG and PNG favicon links plus route-aware title, description, canonical, Open Graph, and Twitter metadata for `/` and `/studio`. `npm run repo:check` and `git diff --check` passed; local browser head checks confirmed the expected tags on both routes | Tests, typecheck, production build, deployment, and crawler/social-card validation remain deferred |
