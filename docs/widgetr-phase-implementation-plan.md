...
| Local branch | `main` |
| Local and remote commit | `c0bcc77 = origin/main` (the live Bitcoin starter and its related UI, export, tests, and documentation are committed and pushed; the current working tree adds the site favicon and route metadata pass, shared viewer-locale formatting, and generic single-line text fitting) |
| Latest verified UI commit | `c0bcc77` (the pushed live Bitcoin starter); the current working-tree metadata and display revision is browser-verified on `/` and `/studio`, including Weather and Bitcoin previews, but remains uncommitted |
