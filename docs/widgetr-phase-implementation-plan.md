...
| Local branch | `main` |
| Local and remote commit | `main` and `origin/main` are synchronized; the homepage WebMCP handoff implementation is in `9b37b26` |
| Latest verified UI commit | `9b37b26` (homepage readiness row, page-action disclosure, provider-neutral handoff, short prompt, and Studio routing); the production rollout is not yet verified |
| Production URL | [`widgetr.vercel.app`](https://widgetr.vercel.app/) |
| Next action | Verify the changed homepage in the rendered local browser when browser control is available, then verify the deployed WebMCP, export, and iPhone gates |
